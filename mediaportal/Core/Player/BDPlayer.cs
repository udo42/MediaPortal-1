  ...
  internal class BDPlayer : IPlayer, IBDReaderCallback
  {
    ...
    protected void DoGraphRebuild()
    {
      if (_mediaCtrl != null)
      {
        firstinit = false;
        FilterState state;
        int hr = _mediaCtrl.GetState(1000, out state);
        ...
        {
          Log.Error("BDPlayer: Error checking graph state: {0}", error.Message);
        }
        ...
        {
          DirectShowUtil.RenderUnconnectedOutputPins(_graphBuilder, VideoCodec);
        ...
        if (_mChangedMediaType != MediaTypeBD.Video)
          DirectShowUtil.RenderUnconnectedOutputPins(_graphBuilder, AudioCodec);
    ...
    protected void CloseInterfaces()
    {
      ...
      try
      {
        ...
        PostProcessFilterVideo.Clear();
        Log.Info("BDPlayer: Cleanup PostProcess");
