  ...
  public class RemovableDriveHelper
  {
    ...
    private static bool DeviceNew(DEV_BROADCAST_VOLUME volumeInformation)
    {
      ...
      else if (Utils.IsDVD(path))
      {
        ...
        {
          Log.Debug("Ignoring volume inserted event - drive {0} - timespan examineCD {1} s - timespan mount {2} s - timespan volume removal {3} s",
    ...
    private static bool DeviceRemoved(DEV_BROADCAST_VOLUME volumeInformation)
    {
      ...
      else if (Utils.IsDVD(path))
      {
        ...
        { 
          Log.Debug("Ignoring volume removed event - drive {0} - time after examineCD {1} s - time after Mount {2} s - time after VolumeInsert {3} s",
