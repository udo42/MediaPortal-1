...
namespace TvPlugin
{
  abstract public class RecordedBase: WindowPluginBase, IComparer<GUIListItem>
  {
    ...
    protected SortMethod _currentSortMethod = SortMethod.Date;
    protected DBView _currentDbView = DBView.Recordings;
    protected static Recording _oActiveRecording = null;
    protected static bool _bIsLiveRecording = false;
    protected bool _deleteWatchedShows = false;
    protected int _iSelectedItem = 0;
    protected string _currentLabel = string.Empty;
    protected int _rootItem = 0;
    protected bool _resetSMSsearch = false;
    protected bool _oldStateSMSsearch;
    protected DateTime _resetSMSsearchDelay;
    ...
    protected abstract string SkinPrefix { get; }

    protected abstract override string SerializeName { get; }

    protected abstract bool TVRecordings { get; } //true will show TV recordings, false radio

    protected abstract string ThumbLocation { get; }

    protected abstract g_Player.MediaType MediaType { get; }
    ...
    /// </summary>
    ///<returns></returns>
    protected static bool ShowFullScreenWindowVideoHandler()
    {
    ...
    protected static void ShowUpcomingEpisodes(Recording rec)
    {
      try
      {
    ...
    protected string GetSpokenViewDate(DateTime aStartTime)
    {
        ...
        if (_currentLabel == string.Empty)
        {
          foreach (Recording rec in recordings)
          {
            ...
            try
            {
              ...
              if (isRadioChannel == !TVRecordings) continue;
              bool add = true;
    ...
    protected GUIListItem BuildItemFromRecording(Recording aRecording)
    {
      ...
      try
      {
        ...
        string StationLogo = Utils.GetCoverArt(ThumbLocation, strChannelName);
    ...
    protected void OnDeleteRecording(int iItem)
    {
      _iSelectedItem = GetSelectedItemNo();
    ...
    protected void DeleteRecordingAndUpdateGUI(Recording rec)
    {
    ...
    protected void TryDeleteRecordingAndNotifyUser(Recording rec)
    {
      TvServer server = new TvServer();
      int timeout = 0;
    ...
    protected void DeleteWatchedRecordings(string currentTitle)
    {
      RemoteControl.Instance.DeleteWatchedRecordings(currentTitle);
    }

    protected void DeleteInvalidRecordings()
    {
    ...
    protected void SetProperties(Recording rec)
    {
      try
      {
        if (rec == null)
        ...
        strLogo = Utils.GetCoverArt(ThumbLocation, GetRecordingDisplayName(rec));
    ...
    protected void SortChanged(object sender, SortEventArgs e)
    {
    ...
    protected void doOnPlayBackStoppedOrChanged(g_Player.MediaType type, int stoptime, string filename, string caller)
    {
    ...
    protected void OnPlayRecordingBackChanged(MediaPortal.Player.g_Player.MediaType type, int stoptime, string filename)
    {
      doOnPlayBackStoppedOrChanged(type, stoptime, filename, "OnPlayRecordingBackChanged");
    }

    protected void OnPlayRecordingBackStopped(g_Player.MediaType type, int stoptime, string filename)
    {
      doOnPlayBackStoppedOrChanged(type, stoptime, filename, "OnPlayRecordingBackStopped");
    }

    protected void OnPlayRecordingBackEnded(g_Player.MediaType type, string filename)
    {
      if (type != MediaType)
      {
        return;
    ...
    protected void OnPlayRecordingBackStarted(g_Player.MediaType type, string filename)
    {
      if (type != MediaType)
      {
        return;
    ...
    protected void ResetWatchedStatus(Recording aRecording)
    {
