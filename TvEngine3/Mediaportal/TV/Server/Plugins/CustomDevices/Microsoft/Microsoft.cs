...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.Microsoft
{
  ...
  public class Microsoft : BaseCustomDevice, IPidFilterController, IDiseqcDevice
  {
    ...
    {
      this.LogDebug("check for IBDA_FrequencyFilter.put_Range() DiSEqC 1.0 support");

      IBDA_Topology topology = filter as IBDA_Topology;
      if (topology == null)
      {
        this.LogDebug("filter is not a topology");
        return null;
      }
      ...
      this.LogDebug("failed to get the control interface, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
      if (controlNode != null)
      {
    ...
    {
      this.LogDebug("check for QAM tuning support");

      IPin pin = DsFindPin.ByDirection(filter, PinDirection.Output, 0);
      if (pin == null)
      {
        this.LogDebug("failed to find output pin");
        return null;
      }
      ...
      if (ps == null)
      {
        this.LogDebug("output pin is not a property set");
        DsUtils.ReleaseComObject(pin);
        pin = null;
        return null;
      }
    ...
    {
      this.LogDebug("check for IMPEG2PIDMap PID filtering support");
      ...
      this.LogDebug("tuner does not implement the interface");
      return null;
    }
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      if (tunerType == CardType.DvbS)
      {
        ...
        if (_diseqcPropertySet != null)
        {
          this.LogDebug("supported device detected (IBDA_DiseqCommand DiSEqC)");
          _isMicrosoft = true;
        }
        else
        {
          ...
          if (_oldDiseqcInterface != null)
          {
            this.LogDebug("supported device detected (IBDA_FrequencyFilter.put_Range() DiSEqC)");
            _isMicrosoft = true;
          }
        }
      }
      ...
      else if (tunerType == CardType.Atsc)
      {
        ...
        if (_qamPropertySet != null)
        {
          this.LogDebug("supported device detected (QAM tuning)");
          _isMicrosoft = true;
        }
      }
      ...
      if (_pidFilterInterface != null)
      {
        this.LogDebug("supported device detected (PID filtering)");
        _isMicrosoft = true;
      }

      if (!_isMicrosoft)
      {
        this.LogDebug("no interfaces supported");
        return false;
      }
    ...
    public override void OnBeforeTune(ITVCard tuner, IChannel currentChannel, ref IChannel channel, out DeviceAction action)
    {
      ...
      if (atscChannel != null && _qamPropertySet != null)
      {
        if (atscChannel.ModulationType == ModulationType.Mod64Qam || atscChannel.ModulationType == ModulationType.Mod256Qam)
        {
          ...
          if (hr != 0)
          {
            this.LogDebug("failed to set QAM modulation, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
          }
          else
          {
    ...
    public bool SetFilterPids(HashSet<UInt16> pids, ModulationType modulation, bool forceEnable)
    {
      ...
      if (toRemove.Count > 0)
      {
        ...
        while (en.MoveNext())
        {
          ...
          if (hr != 0)
          {
            this.LogDebug("failed to remove PID {0} (0x{0:x}), hr = 0x{1:x} ({2})", en.Current, hr, HResult.GetDXErrorString(hr));
            success = false;
          }
          else
          {
    ...
    public bool SetToneState(ToneBurst toneBurstState, Tone22k tone22kState)
    {
      ...
      if (_diseqcPropertySet == null)
      {
        this.LogDebug("the interface does not support setting the tone state");
        return false;
      }
    ...
    public bool SendCommand(byte[] command)
    {
        ...
        command[2] == (byte)DiseqcCommand.WriteN0)
      {
        ...
        this.LogDebug("DiSEqC 1.0 command recognised for port {0}", portNumber);
      }
      ...
      if (_diseqcPropertySet != null)
      {
        ...
        if (hr != 0)
        {
          this.LogDebug("failed to disable DiSEqC command repeats, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
          success = false;
        }
        ...
        if (hr != 0)
        {
          this.LogDebug("failed to disable tone burst commands, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
          success = false;
        }
        ...
        if (portNumber > 0)
        {
          ...
          if (hr != 0)
          {
            this.LogDebug("failed to set LNB source, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
            success = false;
          }
        }
        else
      ...
      else if (_oldDiseqcInterface != null)
      {
        ...
        this.LogDebug("range = 0x{0:x4}", portNumber);
        hr = _oldDiseqcInterface.put_Range((ulong)portNumber);
        if (hr != 0)
        {
          this.LogDebug("failed to put range, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
          success = false;
        }
      }
    ...
    public bool ReadResponse(out byte[] response)
    {
      ...
      if (_diseqcPropertySet == null)
      {
        this.LogDebug("the interface does not support reading DiSEqC responses");
        return false;
      }
