...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.DigitalEverywhere
{
  ...
  public class DigitalEverywhere : BaseCustomDevice, IPowerDevice, IPidFilterController, ICustomTuner, IConditionalAccessProvider, ICiMenuActions, IDiseqcDevice
  {
    ...
    {
      this.LogDebug("read hardware/firmware information");
    ...
    {
      this.LogDebug("read temperature");
    ...
    {
      this.LogDebug("read front end status information");
    ...
    {
      this.LogDebug("request application information");
    ...
    private void MmiHandler()
    {
      ...
      try
      {
        while (!_stopMmiHandlerThread)
        {
          ...
          {
            this.LogDebug("MMI data available, sending request");
            ...
            if (hr != 0)
            {
              this.LogDebug("request failed, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
              continue;
            }

            this.LogDebug("retrieving data");
            for (int i = 0; i < CaDataSize; i++)
            {
            ...
            if (hr != 0)
            {
              this.LogDebug("failed to retrieve data, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
              continue;
            }

            this.LogDebug("handling data");
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("tuner filter is not a property set");
        return false;
      }
      ...
      if (hr != 0)
      {
        this.LogDebug("device does not support the Digital Everywhere property set, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
    ...
    public bool SetPowerState(bool powerOn)
    {
      ...
      {
        this.LogDebug("property not supported, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
    ...
    public bool SetFilterPids(HashSet<UInt16> pids, ModulationType modulation, bool forceEnable)
    {
      ...
      else
      {
        ...
        {
          this.LogDebug("too many PIDs, hardware limit = {0}, actual count = {1}", MaxPidFilterPids, pids.Count);
    ...
    public bool Tune(IChannel channel)
    {
      ...
      if (dvbsChannel != null && _tunerType == CardType.DvbS)
      {
        ...
        if (hr != 0)
        {
          this.LogDebug("failed to apply LNB settings, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        }
    ...
    public bool IsInterfaceReady()
    {
      ...
      this.LogDebug("CI state = {0}", ciState.ToString());
      bool camReady = false;
      if ((ciState & DeCiState.CamPresent) != 0 &&
        (ciState & DeCiState.CamIsDvb) != 0 &&
