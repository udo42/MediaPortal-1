  ...
  public class TechnoTrend : BaseCustomDevice, ICustomTuner, IPowerDevice, IConditionalAccessProvider, ICiMenuActions, IDiseqcDevice
  {
    ...
    private void ReadDeviceInfo()
    {
      ...
      if (result == TtApiResult.Success)
      {
        ...
        /*this.LogDebug("{0}", names.AnalogTunerFilterName);
        this.LogDebug("{0}", names.AnalogCaptureFilterName);
        this.LogDebug("{0}", names.StbCaptureFilterName);*/
        _name = names.ProductName;
      }
      else
      {
        this.LogDebug("failed to read the device details, result = {0}", result);
      }
      ...
      {
        this.LogDebug("failed to determine the product (re)seller, result = {0}", result);
      }
      ...
      {
        this.LogDebug("failed to read the driver version, result = {0}", result);
      }
      ...
      {
        this.LogDebug("failed to read the MAC address, result = {0}", result);
      }
      ...
      if (_deviceCategory == TtDeviceCategory.Usb2 || _deviceCategory == TtDeviceCategory.Usb2Pinnacle || _deviceCategory == TtDeviceCategory.Usb2Dss)
      {
        ...
        {
          this.LogDebug("failed to determine whether USB high speed is supported, result = {0}", result);
        }
      }
    }
    ...
    private void GetCiSlotStatus()
    {
      ...
      if (result != TtApiResult.Success)
      {
        this.LogDebug("bdaapiCIGetSlotStatus failed, result = {0}", result);
      }
    }
    ...
    {
      this.LogDebug("CI slot status callback, slot = {0}", slotIndex);
      if (state == _ciState)
      {
        // Don't be too verbose - we don't need to print the CAS IDs all the time.
        this.LogDebug("CI state = {0}", _ciState);
        return;
      }

      this.LogDebug("CI state change, old state = {0}, new state = {1}", _ciState, state);
      ...
      if (slotInfo == IntPtr.Zero)
      {
        ...
        {
          this.LogDebug("detailed slot info is not available [yet]");
        }
        return;
      }
      ...
      {
        this.LogDebug("CI slot status callback exception\r\n{0}", ex.ToString());
      }
    }
    ...
    {
      this.LogDebug("CA status callback, slot = {0}, reply = {1}, error = {2}", slotIndex, reply, error);
      try
      {
      ...
      {
        this.LogDebug("CA status callback exception\r\n{0}", ex.ToString());
      }
    }
    ...
    private void OnDisplayString(IntPtr context, byte slotIndex, IntPtr text, Int16 textLength)
    {
      try
      {
        ...
        this.LogDebug("display string callback, slot = {0}, string = {1}", slotIndex, _camInputRequestContext);
      }
      catch (Exception ex)
      {
        this.LogDebug("display string callback exception\r\n{0}", ex.ToString());
      }
    }
    ...
    private void OnDisplayMenuOrList(IntPtr context, byte slotIndex, Int16 numEntries, IntPtr entries, Int16 totalMenuLength)
    {
      try
      {
        this.LogDebug("display menu/list callback, slot = {0}, total menu length = {1}", slotIndex, totalMenuLength);
      ...
      {
        this.LogDebug("display menu/list callback exception\r\n{0}", ex.ToString());
      }
    }
    ...
    {
      this.LogDebug("switch OSD off callback, slot = {0}", slotIndex);
      if (_ciMenuCallbacks != null)
      {
        try
        {
        ...
        {
          this.LogDebug("switch OSD off callback exception: {0}", ex.ToString());
        }
      }
      else
      {
    ...
    {
      this.LogDebug("input request callback, slot = {0}", slotIndex);
      ...
      if (_ciMenuCallbacks != null)
      {
        ...
        {
          this.LogDebug("input request callback exception\r\n{0}", ex.ToString());
        }
      }
      else
      {
    ...
    {
      this.LogDebug("OnLscSetDescriptor callback, slot = {0}", slotIndex);
    }
    ...
    {
      this.LogDebug("OnLscConnect callback, slot = {0}", slotIndex);
    }
    ...
    {
      this.LogDebug("OnLscDisconnect callback, slot = {0}", slotIndex);
    }
    ...
    {
      this.LogDebug("OnLscSetParams callback, slot = {0}, buffer size = {1}, timeout = {2}", slotIndex, bufferSize, timeout);
    }
    ...
    {
      this.LogDebug("OnLscEnquireStatus callback, slot = {0}", slotIndex);
    }
    ...
    {
      this.LogDebug("OnLscGetNextBuffer callback, slot = {0}, phase = {1}", slotIndex, phaseId);
    }
    ...
    {
      this.LogDebug("OnLscTransmitBuffer callback, slot = {0}, phase = {1}", slotIndex, phaseId);
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("device category is unknown");
        return false;
      }
      ...
      if (deviceId == -1)
      {
        this.LogDebug("failed to determine device ID");
        return false;
      }
      ...
      {
        this.LogDebug("hardware interface could not be opened");
        return false;
      }

      this.LogDebug("supported device detected, category = {0}, id = {1}", _deviceCategory, deviceId);
      _isTechnoTrend = true;
      ...
      if (_tunerType == CardType.DvbT)
      {
        ...
        if (result != TtApiResult.Success)
        {
          this.LogDebug("failed to turn off auto offset mode, result = {0}", result);
        }
      }
      return true;
    }
    ...
    {
      this.LogDebug("set power state, on = {0}", powerOn);
      ...
      if (_tunerType != CardType.DvbT)
      {
        this.LogDebug("power control is not supported for this device");
        return false;
      }
    ...
    public bool OpenInterface()
    {
      ...
      else
      {
        ...
        this.LogDebug("CI slot not present, result = {0}", result);
    ...
    public bool SendCommand(IChannel channel, CaPmtListManagementAction listAction, CaPmtCommand command, Pmt pmt, Cat cat)
    {
      ...
      this.LogDebug("service ID is {0} (0x{0:x})", pmt.ProgramNumber);
      if (pmt.ProgramNumber == 0)
      {
        this.LogDebug("service 0 cannot be descrambled");
        return false;
      }
    ...
    {
      this.LogDebug("close menu (not implemented)");
      return true;
    }
