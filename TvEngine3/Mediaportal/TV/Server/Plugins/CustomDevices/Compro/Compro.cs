...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.Compro
{
  public class Compro : BaseCustomDevice, IPowerDevice, IDiseqcDevice
  {
    ...
    private void ReadDeviceInfo()
    {
      ...
      // MAC address.
      this.LogDebug("reading MAC address");
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("device does not support the Compro property set, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
    ...
    public bool SetToneState(ToneBurst toneBurstState, Tone22k tone22kState)
    {
      ...
      if (toneBurstState != ToneBurst.None)
      {
        ...
        if (hr != 0 || (support & KSPropertySupport.Set) == 0)
        {
          this.LogDebug("DiSEqC basic property not supported, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        }
        else
        {
          ...
          if (hr != 0)
          {
            this.LogDebug("failed to set tone state, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
            success = false;
          }
        }
      }
      ...
      if (hr != 0 || (support & KSPropertySupport.Set) == 0)
      {
        this.LogDebug("tone/power property not supported, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
      }
      else
      {
        ...
        if (hr != 0)
        {
          this.LogDebug("failed to set 22 kHz state, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
          success = false;
        }
      }
