...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.Geniatech
{
  ...
  public class Geniatech : Conexant.Conexant, IPowerDevice
  {
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("device does not support the NBC parameter property, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
    ...
    public override void OnBeforeTune(ITVCard tuner, IChannel currentChannel, ref IChannel channel, out DeviceAction action)
    {
      ...
      if (hr != 0)
      {
        this.LogDebug("failed to set pilot and roll-off, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
      }
    }
    ...
    public bool SetPowerState(bool powerOn)
    {
      ...
      if (hr != 0 || (support & KSPropertySupport.Set) == 0)
      {
        this.LogDebug("LNB power property not supported, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
      }
