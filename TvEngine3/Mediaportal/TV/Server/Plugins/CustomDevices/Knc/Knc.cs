...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.Knc
{
  ...
  public class Knc : BaseCustomDevice, IConditionalAccessProvider, ICiMenuActions, IDiseqcDevice
  {
    ...
    {
      this.LogDebug("device {0} swap CI slots", _deviceIndex);
      ...
      if (!_isPcie)
      {
        this.LogDebug("function not supported");
        return false;
      }
      ...
      this.LogDebug("main device count is {0}", num716xDevices);
      if (num716xDevices == 0)
      {
        return false;
      }
      ...
      {
        this.LogDebug("failed to open main device");
        return false;
      }
    ...
    {
      this.LogDebug("device {0} CI state change callback, slot = {1}", _deviceIndex, slotIndex);
      lock (this)
      {
    ...
    {
      this.LogDebug("device {0} open menu callback, slot = {1}", _deviceIndex, slotIndex);
    }
    ...
    {
      this.LogDebug("device {0} menu callback, slot = {1}", _deviceIndex, slotIndex);
      ...
      if (_ciMenuCallbacks != null)
      {
        ...
        {
          this.LogDebug("menu callback exception\r\n{0}", ex.ToString());
        }
      }
      else
      {
    ...
    {
      this.LogDebug("device {0} menu entry callback, slot = {1}", _deviceIndex, slotIndex);
    ...
    {
      this.LogDebug("device {0} request callback, slot = {1}", _deviceIndex, slotIndex);
      ...
      if (_ciMenuCallbacks != null)
      {
        ...
        {
          this.LogDebug("request callback exception\r\n{0}", ex.ToString());
        }
      }
      else
      {
    ...
    {
      this.LogDebug("device {0} close menu callback, slot = {1}, delay = {2}", _deviceIndex, slotIndex, delay);
      if (_ciMenuCallbacks != null)
      {
        try
        {
        ...
        {
          this.LogDebug("close menu callback exception\r\n{0}", ex.ToString());
        }
      }
      else
      {
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      if (hr != 0)
      {
        this.LogDebug("failed to get the tuner filter name, hr = 0x{0:x} - {1}", HResult.GetDXErrorString(hr));
        return false;
      }
      foreach (String validTunerName in ValidDeviceNames)
      {
        ...
        {
          this.LogDebug("recognised filter name \"{0}\"", tunerInfo.achName);
          ...
          if (_deviceIndex < 0)
          {
            this.LogDebug("failed to calculate the device index");
            break;
          }

          // Stage 3: ensure we can get a reference to the filter that implements the proprietary
          // interfaces.
          this.LogDebug("device index is {0}", _deviceIndex);
          ...
          this.LogDebug("this is a PCIe device");
          _isPcie = true;
          _name = "KNC";
          ...
          {
            this.LogDebug("failed to get the capture filter, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
          }
          else
          {
            _isKnc = true;
      ...
      if (!_isKnc)
      {
        this.LogDebug("device not supported");
      ...
      {
        this.LogDebug("failed to enable the hardware");
      }
      return true;
    }
    ...
    {
      this.LogDebug("device {0} open conditional access interface", _deviceIndex);
      ...
      if (!result)
      {
        this.LogDebug("CI enable failed");
        return false;
      }
      ...
      {
        this.LogDebug("CI HW enable failed");
        return false;
      }

      this.LogDebug("CI interface opened successfully");
      ...
      if (_isCamPresent)
      {
        ...
        if (_isCamReady)
        {
          ...
          {
            this.LogDebug("CAM name/type is {0}", nameBuffer);
          }
          else
          {
            this.LogDebug("failed to get the CAM name/type");
          }
        }
      }
    ...
    {
      this.LogDebug("device {0} close conditional access interface", _deviceIndex);
      ...
      if (_callbackBuffer != IntPtr.Zero)
      {
        ...
        {
          this.LogDebug("CI disable failed");
          result = false;
        }
    ...
    public bool IsInterfaceReady()
    {
      ...
      {
        this.LogDebug("CI slot or CAM not present");
        return false;
      }

      bool camReady = KNCBDA_CI_IsReady(_deviceIndex);
      this.LogDebug("result = {0}", camReady);
      return camReady;
    }
    ...
    {
      this.LogDebug("device {0} slot {1} enter menu", _deviceIndex, _slotIndex);
      if (!_isKnc)
      {
        this.LogDebug("device not initialised or interface not supported");
        return false;
    ...
    {
      this.LogDebug("device {0} slot {1} close menu", _deviceIndex, _slotIndex);
      if (!_isKnc)
      {
        this.LogDebug("device not initialised or interface not supported");
        return false;
    ...
    {
      this.LogDebug("device {0} slot {1} select menu entry, choice = {2}", _deviceIndex, _slotIndex, choice);
      if (!_isKnc)
      {
        this.LogDebug("device not initialised or interface not supported");
        return false;
    ...
    public bool SendMenuAnswer(bool cancel, String answer)
    {
      ...
      this.LogDebug("device {0} slot {1} send menu answer, answer = {2}, cancel = {3}", _deviceIndex, _slotIndex, answer, cancel);
      if (!_isKnc)
      {
        this.LogDebug("device not initialised or interface not supported");
        return false;
    ...
    {
      this.LogDebug("device {0} send DiSEqC command", _deviceIndex);
