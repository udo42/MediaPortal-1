  ...
  public class Anysee : BaseCustomDevice, IConditionalAccessProvider, ICiMenuActions, IDiseqcDevice 
  {
    ...
    private class AnyseeCiApi
    {
      ...
      public AnyseeCiApi()
      {
        _apiCount++;
        _apiIndex = _apiCount;
        this.LogDebug("loading API, API index = {0}", _apiIndex);
        if (!File.Exists("Plugins\\CustomDevices\\Resources\\CIAPI" + _apiIndex + ".dll"))
        {
          ...
          {
            this.LogDebug("failed to copy CIAPI.dll\r\n{0}", ex.ToString());
            return;
          }
        }
        ...
        {
          this.LogDebug("failed to load the DLL");
          return;
        }
        ...
        try
        {
          ...
          {
            this.LogDebug("failed to locate the CreateDtvCIAPI function");
            return;
          }
          try
          {
          ...
          {
            this.LogDebug("failed to load the CreateDtvCIAPI function\r\n{0}", ex.ToString());
            return;
          }
          ...
          if (function == IntPtr.Zero)
          {
            this.LogDebug("failed to locate the DestroyDtvCIAPI function");
            return;
          }
          try
          {
          ...
          {
            this.LogDebug("failed to load the DestroyDtvCIAPI function\r\n{0}", ex.ToString());
            return;
          }
          ...
          if (function == IntPtr.Zero)
          {
            this.LogDebug("failed to locate the GetanyseeNumberofDevicesEx function");
            return;
          }
          try
          {
          ...
          {
            this.LogDebug("failed to load the GetanyseeNumberofDevicesEx function\r\n", ex.ToString());
            return;
          }
          ...
          if (function == IntPtr.Zero)
          {
            this.LogDebug("failed to locate the OpenCILib function");
            return;
          }
          try
          {
          ...
          {
            this.LogDebug("failed to load the OpenCILib function\r\n{0}", ex.ToString());
            return;
          }
          ...
          if (function == IntPtr.Zero)
          {
            this.LogDebug("failed to locate the CI_Control function");
            return;
          }
          try
          {
          ...
          {
            this.LogDebug("failed to load the CI_Control function\r\n{0}", ex.ToString());
            return;
          }
      ...
      {
        this.LogDebug("opening API, API index = {0}", _apiIndex);

        if (!_dllLoaded)
        {
          this.LogDebug("the CIAPI.dll functions were not successfully loaded");
          return false;
        }
        if (_apiAccessThread != null && _apiAccessThread.IsAlive)
        {
          this.LogDebug("API access thread is already running");
          return false;
        }
        ...
        this.LogDebug("starting API access thread");
        ...
        {
          this.LogDebug("API access thread running");
          return true;
        }
        this.LogDebug("API access thread self-terminated");
        return false;
      }
      ...
      {
        this.LogDebug("creating new CI API instance");
        int result = _createApi(_ciApiInstance);
        if (result != 1)
        {
          this.LogDebug("failed to create instance, result = {0}", result);
          return;
        }
        this.LogDebug("created instance successfully");

        // We have an API instance, but now we need to open it by linking it with hardware.
        this.LogDebug("determining instance index");
        ...
        this.LogDebug("number of devices = {0}", numDevices);
        if (numDevices == 0)
        {
        ...
        for (int i = 0; i < numDevices; i++)
        {
          ...
          {
            this.LogDebug("found correct instance");
        ...
        if (index != -1)
        {
          ...
          {
            this.LogDebug("result = failure, hr = 0x{0:x} ({1})", result, HResult.GetDXErrorString(result));
          }
        }

        // When this thread is stopped, we automatically destroy the API instance.
        this.LogDebug("destroying CI API instance");
      ...
      {
        this.LogDebug("closing API");

        if (!_dllLoaded)
        {
          this.LogDebug("the CIAPI.dll functions have not been loaded");
          return true;
        }
        ...
        {
          this.LogDebug("API access thread is null");
        }
        else
        {
          this.LogDebug("API access thread state = {0}", _apiAccessThread.ThreadState);
        }
      ...
      {
        this.LogDebug("execute API command");
        ...
        if (!_apiAccessThread.IsAlive)
        {
          this.LogDebug("the API is not open");
          return false;
        }
    ...
    {
      this.LogDebug("read NIM configuration");
    ...
    {
      this.LogDebug("read driver version");
    ...
    {
      this.LogDebug("read platform information");
    ...
    {
      this.LogDebug("read board information");
    ...
    {
      this.LogDebug("read capabilities");
    ...
    {
      this.LogDebug("send key, key = {0}", key);
      if (_ciApi == null)
      {
        this.LogDebug("the conditional access interface is not open");
        return false;
      }
    ...
    private Int32 OnCiState(Int32 slotIndex, AnyseeCiState state, String message)
    {
      ...
      this.LogDebug("CI state change callback, slot = {0}", slotIndex);
    ...
    {
      this.LogDebug("MMI message callback, slot = {0}", slotIndex);
      ...
      {
        this.LogDebug("enquiry");
        if (msg.HeaderCount != 1)
        {
          this.LogDebug("unexpected header count, count = {0}", msg.HeaderCount);
          return 1;
        }
        ...
        if (_ciMenuCallbacks != null)
        {
          ...
          {
            this.LogDebug("MMI callback enquiry exception\r\n{0}", ex.ToString());
            return 1;
          }
        }
        return 0;
      ...
      if (_ciMenuCallbacks != null)
      {
        ...
        {
          this.LogDebug("MMI callback header exception\r\n{0}", ex.ToString());
          return 1;
        }
      }
      ...
      for (int i = 0; i < msg.EntryCount; i++)
      {
        ...
        if (_ciMenuCallbacks != null)
        {
          ...
          {
            this.LogDebug("MMI callback entry exception\r\n{0}", ex.ToString());
            return 1;
          }
        }
      }
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("failed to find the tuner filter output pin");
        return false;
      }
      ...
      {
        this.LogDebug("failed to get the capture filter input pin, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
      ...
      if (hr != 0)
      {
        this.LogDebug("failed to get the capture filter input pin info, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
      ...
      {
        this.LogDebug("capture filter is not a property set");
        return false;
      }
      ...
      if (hr != 0 || support == 0)
      {
        this.LogDebug("device does not support the Anysee property set, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
    ...
    public bool OpenInterface()
    {
      ...
      if (_ciApi != null)
      {
        this.LogDebug("previous interface instance is still open");
        return false;
      }
      ...
      this.LogDebug("setting callbacks");
    ...
    public bool SendCommand(IChannel channel, CaPmtListManagementAction listAction, CaPmtCommand command, Pmt pmt, Cat cat)
    {
      ...
      foreach (IDescriptor d in pmt.ProgramCaDescriptors)
      {
        ...
        {
          this.LogDebug("PMT program CA descriptor data is too long");
          return false;
        }
      ...
      // Elementary streams.
      this.LogDebug("elementary streams");
      ...
      foreach (PmtElementaryStream es in pmt.ElementaryStreams)
      {
        ...
        foreach (IDescriptor d in es.CaDescriptors)
        {
          ...
          if (offset + descriptorData.Count >= MaxDescriptorDataLength)
          {
            this.LogDebug("PMT elementary stream {0} (0x{0:x}) CA data is too long", es.Pid);
            return false;
          }
        ...
        {
          this.LogDebug("reached maximum number of included PIDs");
          break;
        }
      }

      this.LogDebug("total included PIDs = {0}", esCount);
    ...
    public bool SendMenuAnswer(bool cancel, String answer)
    {
      ...
      for (int i = 0; i < answer.Length; i++)
      {
        ...
        {
          this.LogDebug("answer may only contain numeric digits");
          return false;
        }
