...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.MdPlugin
{
  ...
  public class MdPlugin : BaseCustomDevice, IAddOnDevice, IConditionalAccessProvider
  {
    ...
    {
      this.LogDebug("registering video and audio PIDs");
      ...
      foreach (PmtElementaryStream es in pmt.ElementaryStreams)
      {
        ...
        {
          this.LogDebug("unable to register all PIDs");
          break;
        }
    ...
    {
      this.LogDebug("registering ECM and EMM details");
      ...
      this.LogDebug("reading PMT program CA descriptors...");
      ...
      while (descEn.MoveNext())
      {
        ...
        if (cad == null)
        {
          this.LogDebug("invalid descriptor");
      ...
      this.LogDebug("merging PMT elementary stream CA descriptors...");
      ...
      this.LogDebug("reading CAT CA descriptors...");
      ...
      this.LogDebug("registering PIDs...");
    ...
    private void SetPreferredCaSystemIndex(ref Program82 programToDecode)
    {
      try
      {
        this.LogDebug("identifying primary ECM PID");
        ...
        if (File.Exists(configFile))
        {
          ...
          if (channelList != null)
          {
            ...
            foreach (XmlNode channelNode in channelList)
            {
              ...
              {
                this.LogDebug("found channel configuration");
                for (byte i = 0; i < programToDecode.CaSystemCount; i++)
                {
                  ...
                  {
                    this.LogDebug("found correct ECM PID");
            ...
            if (providerList != null)
            {
              foreach (XmlNode providerNode in providerList)
              {
                for (byte i = 0; i < programToDecode.CaSystemCount; i++)
                {
                  ...
                  {
                    this.LogDebug("found provider configuration");
                    programToDecode.CaId = i;
                    programToDecode.EcmPid = programToDecode.CaSystems[i].EcmPid;
                    configFound = true;
                    break;
            ...
            if (caTypeList != null)
            {
              foreach (XmlNode caTypeNode in caTypeList)
              {
                for (byte i = 0; i < programToDecode.CaSystemCount; i++)
                {
                  ...
                  {
                    this.LogDebug("found CA type configuration");
                    programToDecode.CaId = i;
                    programToDecode.EcmPid = programToDecode.CaSystems[i].EcmPid;
                    configFound = true;
                    break;
        ...
        if (!configFound)
        {
          this.LogDebug("no configuration found");
          ...
          if (fillOutConfig)
          {
            this.LogDebug("attempting to add entries to MDAPIProvID.xml");
      ...
      {
        this.LogDebug("failed to load or read configuration, or set preferred CA system index\r\n{0}", ex.ToString());
      }
    }
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("plugin not configured");
        return false;
      }
      ...
      try
      {
        ...
        if (File.Exists(configFile))
        {
          this.LogDebug("searching for device configuration, device path = {0}", tunerDevicePath);
          doc.Load(configFile);
          ...
          if (rootNode != null)
          {
            ...
            if (tunerList != null)
            {
              this.LogDebug("found {0} device configuration(s)", tunerList.Count);
              foreach (XmlNode node in tunerList)
              {
                if (node.Attributes["DevicePath"].Value.Equals(tunerDevicePath))
                {
                  ...
                  this.LogDebug("found configuration");
                  tunerNode = node;
                  break;
                }
              }
        ...
        if (tunerNode == null)
        {
          this.LogDebug("creating device configuration");
        ...
        if (slotCount > 0)
        {
          this.LogDebug("plugin is enabled for {0} decoding slot(s)", slotCount);
          _isMdPlugin = true;
          _slots = new List<DecodeSlot>(slotCount);
          if (_providers.Count == 0)
          {
            this.LogDebug("plugin can decrypt services from any provider");
          }
          else
          {
            this.LogDebug("plugin can decrypt services from provider(s) \"{0}\"", providerList);
          }
          return true;
        }

        this.LogDebug("plugin is not enabled");
        return false;
      }
      catch (Exception ex)
      {
        this.LogDebug("failed to create, load or read configuration\r\n{0}", ex.ToString());
        return false;
      }
    }
    ...
    public bool AddToGraph(ref IBaseFilter lastFilter)
    {
      ...
      {
        this.LogDebug("{0} device filter(s) already in graph", _slots.Count);
        return true;
      }
      ...
      this.LogDebug("adding infinite tee");
      _infTee = (IBaseFilter)new InfTee();
      hr = _graph.AddFilter(_infTee, "MD Plugin Infinite Tee");
      if (hr != 0)
      {
        this.LogDebug("failed to add the inf tee to the graph, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
      ...
      if (hr != 0)
      {
        this.LogDebug("failed to connect the inf tee into the graph, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
      lastFilter = _infTee;

      // Add one filter for each decoding slot. Note that we preset the capacity in Initialise().
      this.LogDebug("adding {0} decoding filter(s)", _slots.Capacity);
      for (int i = 0; i < _slots.Capacity; i++)
      {
        ...
        if (hr != 0)
        {
          this.LogDebug("failed to add MD plugin filter {0} to the graph, hr = 0x{1:x} ({2})", i + 1, hr, HResult.GetDXErrorString(hr));
          return false;
        }
        ...
        if (hr != 0)
        {
          this.LogDebug("failed to connect MD plugin filter {0} into the graph, hr = 0x{1:x} ({2})", i + 1, hr, HResult.GetDXErrorString(hr));
          return false;
        }
        ...
        try
        {
          ...
          this.LogDebug("plugins directory is \"{0}{1}\"", _configurationFolderPrefix + i);
        }
        catch (Exception ex)
        {
          this.LogDebug("failed to set plugin directory\r\n{0}", ex.ToString());
          return false;
        }
    ...
    public bool IsInterfaceReady()
    {
      ...
      if (_programmeBuffer == IntPtr.Zero)
      {
        this.LogDebug("interface not opened");
        return false;
      }

      // As far as we know, the interface is always ready as long as it is open.
      this.LogDebug("result = {0}", _slots.Count > 0);
    ...
    public bool SendCommand(IChannel channel, CaPmtListManagementAction listAction, CaPmtCommand command, Pmt pmt, Cat cat)
    {
      ...
      if (cat == null)
      {
        this.LogDebug("CAT not supplied");
        return true;
      }
      DVBBaseChannel dvbChannel = channel as DVBBaseChannel;
      if (dvbChannel == null)
      {
        this.LogDebug("channel is not a DVB channel");
        return true;
      }
      if (_providers.Count != 0 && !_providers.Contains(dvbChannel.Provider))
      {
        this.LogDebug("plugin not configured to decrypt services for provider \"{0}\"", dvbChannel.Provider);
        return false;
      }
      ...
      {
        this.LogDebug("freeing all slots");
      ...
      else
      {
        foreach (DecodeSlot slot in _slots)
        {
          ...
          if (currentService != null && currentService.ServiceId == dvbChannel.ServiceId)
          {
            ...
            {
              this.LogDebug("found existing slot decrypting channel \"{0}\", freeing", currentService.Name);
              slot.CurrentChannel = null;
              IChangeChannel_Clear clear = slot.Filter as IChangeChannel_Clear;
              if (clear != null)
              {
            ...
            {
              this.LogDebug("found existing slot decrypting channel \"{0}\", sending update", currentService.Name);
              freeSlot = slot;
      ...
      if (command == CaPmtCommand.NotSelected)
      {
        ...
        this.LogDebug("received \"not selected\" request for channel that is not being decrypted");
        return true;
      }
      ...
      if (freeSlot == null)
      {
        ...
        this.LogDebug("no free decrypt slots");
        return false;
      }
      ...
      this.LogDebug("TSID = {0} (0x{0:x}), SID = {1} (0x{1:x}), PMT PID = {2} (0x{2:x}), PCR PID = {3} (0x{3:x}), service type = {4}, " +
      ...
      this.LogDebug("ECM PID = {0} (0x{0:x}), CA system count = {1}, CA index = {2}",
                    programToDecode.EcmPid, programToDecode.CaSystemCount, programToDecode.CaId
      );
      for (byte i = 0; i < programToDecode.CaSystemCount; i++)
      {
        this.LogDebug("#{0} CA type = {1} (0x{1:x}), ECM PID = {2} (0x{2:x}), EMM PID = {3} (0x{3:x}), provider = {4} (0x{4:x})",
      ...
      {
        this.LogDebug("failed to change channel\r\n{0}", ex.ToString());
        return false;
      }
