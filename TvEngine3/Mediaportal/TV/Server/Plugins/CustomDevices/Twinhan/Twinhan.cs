...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.Twinhan
{
  ...
  public class Twinhan : BaseCustomDevice, /*IAddOnDevice, ICustomTuner,*/ IPowerDevice, IPidFilterController, IConditionalAccessProvider, ICiMenuActions, IDiseqcDevice
  {
    ...
    private void ReadDeviceInfo()
    {
      ...
      //this.LogDebug("number of DeviceInfo bytes returned is {0}", returnedByteCount);
    ...
    {
      this.LogDebug("read PID filter information");
      ...
      //this.LogDebug("number of PidFilterParams bytes returned is {0}", returnedByteCount);
      //DVB_MMI.DumpBinary(_generalBuffer, 0, returnedByteCount);
    ...
    {
      this.LogDebug("read driver information");
      ...
      //this.LogDebug("number of DriverInfo bytes returned is {0}", returnedByteCount);
      //DVB_MMI.DumpBinary(_generalBuffer, 0, returnedByteCount);
    ...
    private void MmiHandler()
    {
      ...
      try
      {
        while (!_stopMmiHandlerThread)
        {
          ...
          {
            this.LogDebug("CI state change, old state = {0}, new state = {1}", prevCiState, ciState);
            prevCiState = ciState;
          ...
          {
            this.LogDebug("MMI state change, old state = {0}, new state = {1}", prevMmiState, mmiState);
          }
            ...
            if (ReadMmi(out mmi))
            {
              if (mmi.IsEnquiry)
              {
                ...
                if (_ciMenuCallbacks != null)
                {
                  ...
                  {
                    this.LogDebug("menu request callback exception\r\n{0}", ex.ToString());
                  }
                }
                else
                {
          ...
          {
            this.LogDebug("menu close request");
            if (_ciMenuCallbacks != null)
            {
              try
              {
    ...
    {
      this.LogDebug("send MMI message");
    ...
    {
      this.LogDebug("read MMI response");
      mmi = new MmiData();
      int hr;
      lock (this)
      {
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      if (hr != 0)
      {
        this.LogDebug("device does not support the Twinhan property set, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
      ...
      else
      {
        ...
        {
          this.LogDebug("this device is using a TerraTec driver");
          _isTerraTec = true;
        }
      }
    ...
    public bool AddToGraph(ICaptureGraphBuilder2 graphBuilder, ref IBaseFilter lastFilter)
    {
      ...
      this.LogDebug("TerraTec H7 compatible device detected");
      ...
      {
        this.LogDebug("filter does not have topology information, assuming filter not required");
        return true;
      }

      if (!captureFilterRequired)
      {
        this.LogDebug("filter is not a tuner filter so a capture filter is not required");
        return true;
      }
    ...
    public bool SetPowerState(bool powerOn)
    {
      ...
      {
        this.LogDebug("function disabled for safety");
    ...
    public bool Tune(IChannel channel)
    {
      ...
      {
        this.LogDebug("tuning not supported");
        return false;
      }
    ...
    public bool SetFilterPids(HashSet<UInt16> pids, ModulationType modulation, bool forceEnable)
    {
      ...
      if (!_isPidFilterSupported)
      {
        this.LogDebug("PID filtering not supported");
        return true;    // Don't retry...
      }
      ...
      else
      {
        ...
        {
          this.LogDebug("too many PIDs, hardware limit = {0}, actual count = {1}", _maxPidFilterPids, pids.Count);
    ...
    public bool OpenInterface()
    {
      ...
      this.LogDebug("update CI/CAM state");
      _isCamPresent = false;
      _isCamReady = false;
    ...
    public bool ResetInterface(out bool rebuildGraph)
    {
      ...
      if (hr != 0)
      {
        this.LogDebug("reset property failed, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        rebuildGraph = true;
        return true;
      }
    ...
    public bool IsInterfaceReady()
    {
      ...
      this.LogDebug("CI state = {0}, MMI state = {1}", ciState, mmiState);
      bool camReady = false;
