  ...
  public class Turbosight : BaseCustomDevice, IPowerDevice, IConditionalAccessProvider, ICiMenuActions, IDiseqcDevice
  {
    ...
    private bool LoadNewCaApiInstance()
    {
      ...
      if (!File.Exists(sourceFilename))
      {
        ...
        {
          this.LogDebug("failed to copy tbsCIapi.dll\r\n{0}", ex.ToString());
          return false;
        }
      }
      ...
      try
      {
        ...
        {
          this.LogDebug("failed to locate the On_Start_CI function");
          return false;
        }
        try
        {
        ...
        {
          this.LogDebug("failed to load the On_Start_CI function\r\n{0}", ex.ToString());
          return false;
        }
        ...
        if (function == IntPtr.Zero)
        {
          this.LogDebug("failed to locate the Camavailable function");
          return false;
        }
        try
        {
        ...
        {
          this.LogDebug("failed to load the Camavailable function\r\n{0}", ex.ToString());
          return false;
        }
        ...
        if (function == IntPtr.Zero)
        {
          this.LogDebug("failed to locate the TBS_ci_MMI_Process function");
          return false;
        }
        try
        {
        ...
        {
          this.LogDebug("failed to load the TBS_ci_MMI_Process function\r\n", ex.ToString());
          return false;
        }
        ...
        if (function == IntPtr.Zero)
        {
          this.LogDebug("failed to locate the TBS_ci_SendPmt function");
          return false;
        }
        try
        {
        ...
        {
          this.LogDebug("failed to load the TBS_ci_SendPmt function\r\n{0}", ex.ToString());
          return false;
        }
        ...
        if (function == IntPtr.Zero)
        {
          this.LogDebug("failed to locate the On_Exit_CI function");
          return false;
        }
        try
        {
        ...
        {
          this.LogDebug("failed to load the On_Exit_CI function\r\n{0}", ex.ToString());
          return false;
        }
      ...
      try
      {
        while (!_stopMmiHandlerThread)
        {
          ...
          if (newState != _isCamPresent)
          {
            ...
            this.LogDebug("CAM state change, CAM present = {0}", _isCamPresent);
          ...
          if (message == TbsMmiMessageType.Null)
          {
            ...
            lock (this)
            {
              ...
              if (_mmiMessageQueue.Count > 0)
              {
                ...
                this.LogDebug("sending message {0}", message);
          ...
          if (message == TbsMmiMessageType.EnterMenu || message == TbsMmiMessageType.MenuAnswer || message == TbsMmiMessageType.Answer || message == TbsMmiMessageType.CloseMmi)
          {
            ...
            if (_mmiMessageQueue.Count == 0)
            {
              this.LogDebug("resuming polling...");
            }
            continue;
          }
          ...
          if (response == TbsMmiMessageType.Null)
          {
            ...
            if (message != TbsMmiMessageType.Null)
            {
              ...
              if (sendCount >= 10)
              {
                ...
                this.LogDebug("giving up on message {0}", message);
                message = TbsMmiMessageType.Null;
                if (_mmiMessageQueue.Count == 0)
                {
                  this.LogDebug("resuming polling...");
          ...
          this.LogDebug("received MMI response {0} to message {1}", response, message);
          ...
          {
            this.LogDebug("response too long, length = {0}", length);
          ...
          this.LogDebug("response length = {0}", length);
          ...
          {
            this.LogDebug("unhandled response message {0}", response);
    ...
    {
      this.LogDebug("application information");
      if (length < 5)
      {
        this.LogDebug("error, response too short");
    ...
    {
      this.LogDebug("conditional access information");
      if (length == 0)
      {
        this.LogDebug("error, response too short");
        return;
      ...
      {
        this.LogDebug("error, unexpected numCasIds");
        DVB_MMI.DumpBinary(_mmiResponseBuffer, 0, length);
      }
    }
    ...
    private void HandleMenu(byte[] content, int length)
    {
      ...
      if (entryCount < 0)
      {
        this.LogDebug("error, not enough menu entries");
        DVB_MMI.DumpBinary(content, 0, length);
        return;
      }
      ...
      for (int i = 0; i < entryCount; i++)
      {
        ...
        if (_ciMenuCallbacks != null)
        {
          ...
          {
            this.LogDebug("menu entry callback exception\r\n{0}", ex.ToString());
          }
        }
      }
      ...
      {
        this.LogDebug("error, numEntries != entryCount");
        //DVB_MMI.DumpBinary(_mmiResponseBuffer, 0, length);
      }
    }
    ...
    private void HandleEnquiry(byte[] content, int length)
    {
      ...
      if (_ciMenuCallbacks != null)
      {
        ...
        {
          this.LogDebug("CAM request callback exception\r\n{0}", ex.ToString());
        }
      }
      else
      {
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("tuner filter name does not match");
        return false;
      }
      this.LogDebug("checking device named \"{0}\"", _tunerFilterName);
      ...
      if (_propertySet != null)
      {
        ...
        if (hr == 0 && support != 0)
        {
          ...
          this.LogDebug("supported device detected (USB interface)");
          _isTurbosight = true;
          _isUsb = true;
      ...
      if (!_isTurbosight)
      {
        ...
        if (_propertySet != null)
        {
          ...
          if (hr == 0 && support != 0)
          {
            ...
            this.LogDebug("supported device detected (PCIe/PCI interface)");
            _isTurbosight = true;
            _isUsb = false;
      ...
      if (!_isTurbosight)
      {
        this.LogDebug("device does not support interfaces");
        return false;
      }
    ...
    public override void OnBeforeTune(ITVCard tuner, IChannel currentChannel, ref IChannel channel, out DeviceAction action)
    {
      ...
      {
        this.LogDebug("NBC tuning parameter property not supported, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return;
      }
    ...
    public bool OpenInterface()
    {
      ...
      if (hr != 0 || support == 0)
      {
        this.LogDebug("device doesn't have a CI slot");
        return false;
      }
      ...
      {
        this.LogDebug("interface handle is null");
        _isCiSlotPresent = false;
        return false;
      }
      ...
      this.LogDebug("CAM available = {0}", _isCamPresent);
    ...
    public bool CloseInterface()
    {
      ...
      if (_ciHandle != IntPtr.Zero)
      {
        ...
        catch (Exception ex)
        {
          ...
          this.LogDebug("On_Exit_CI threw exception, handle address = {0}\r\n{1}", handleAddress, ex.ToString());
        }
    ...
    public bool SendCommand(IChannel channel, CaPmtListManagementAction listAction, CaPmtCommand command, Pmt pmt, Cat cat)
    {
      ...
      {
        this.LogDebug("buffer capacity too small");
        return false;
      }
    ...
    public bool ReadResponse(out byte[] response)
    {
      ...
      {
        this.LogDebug("result = failure, unexpected number of bytes ({0}) returned", returnedByteCount);
        return false;
      }
      ...
      {
        this.LogDebug("result = failure, unexpected number of message bytes ({0}) returned", accessParams.DiseqcReceiveMessageLength);
        return false;
      }
