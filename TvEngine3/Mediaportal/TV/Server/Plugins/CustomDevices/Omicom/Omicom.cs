...
namespace Mediaportal.TV.Server.Plugins.CustomDevices.Omicom
{
  ...
  public class Omicom : BaseCustomDevice, IDiseqcDevice
  {
    ...
    public override bool Initialise(IBaseFilter tunerFilter, CardType tunerType, String tunerDevicePath)
    {
      ...
      {
        this.LogDebug("device does not support the Omicom property set, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
        return false;
      }
    ...
    public bool ReadResponse(out byte[] response)
    {
      ...
      {
        this.LogDebug("reply too long, length = {0}", message.MessageLength);
        return false;
      }
      this.LogDebug("result = success");
