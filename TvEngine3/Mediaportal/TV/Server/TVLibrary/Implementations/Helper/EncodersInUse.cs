    ...
    public bool Add(DsDevice device, SoftwareEncoder dbEncoder)
    {
      ...
      lock (_encodersInUse)
      {
        ...
        foreach (DsDevice dev in _encodersInUse.Keys)
        {
          ...
          {
            this.LogInfo(" compressor {0} is in use, checking reuse limit...", dev.Name);
            key = dev;
            break;
          }
        }
        ...
        if (key == null)
        {
          this.LogInfo(" compressor {0} is usable", device.Name);
        ...
        if (dbEncoder == null)
        {
          this.LogInfo(" unrecognised compressor, assuming usable");
          _encodersInUse[key]++;
          return true;
        }
        ...
        if (dbEncoder.Reusable)
        {
          ...
          {
            this.LogInfo(" reusable compressor, usage under limit (usage: {0}, limit: {1})",
          ...
          {
            this.LogInfo(" reusable compressor, usage already at limit (usage: {0}, limit: {1})",
      ...
      // if it wasn't in use...
      this.LogInfo(" non-reusable compressor, already used");
      return false;
    }
    ...
    public void Remove(DsDevice device)
    {
      ...
      lock (_encodersInUse)
      {
        foreach (DsDevice dev in _encodersInUse.Keys)
        {
          if (dev.Name == device.Name && device.Mon.IsEqual(dev.Mon) == 0 && dev.DevicePath == device.DevicePath)
          {
            this.LogInfo("removing instance of compressor {0} from use", dev.Name);
            _encodersInUse[dev]--;
            if (_encodersInUse[dev] == 0)
            {
              this.LogInfo("compressor is no longer in use");
