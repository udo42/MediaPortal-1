...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.Analog.Graphs.Analog
{
  ...
  public class AnalogSubChannel : BaseSubChannel, ITvSubChannel, IAnalogTeletextCallBack, IAnalogVideoAudioObserver
  {
    ...
    {
      this.LogDebug("{0} OnBeforeTune", _subChannelId);
      if (IsTimeShifting)
      {
        if (_subChannelId >= 0)
        {
    ...
    {
      this.LogDebug("{0} OnAfterTune", _subChannelId);
      if (IsTimeShifting)
      {
        if (_subChannelId >= 0)
        {
    ...
    {
      this.LogDebug("{0} OnGraphRunning", _subChannelId);
    ...
    protected override void OnStartTimeShifting(string fileName)
    {
      ...
      this.LogDebug("SetTimeShiftFileName:{0}", fileName);
    ...
    {
      this.LogDebug(" StopTimeShifting()");
    ...
    protected override void OnStartRecording(string fileName)
    {
      ...
      this.LogDebug("StartRecord({0})", fileName);
