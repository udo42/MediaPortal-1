...
namespace Mediaportal.TV.Server.TVLibrary.Implementations.Analog.Components
{
  internal class Encoder
  {
    ...
    {
      Log.Debug("analog: ConnectEncoderFilter video:{0} audio:{1}", isVideo, isAudio);
      ...
      try
      {
        ...
        Log.Debug("analog:  pinsAvailable on capture filter:{0}", pinsAvailable);
        for (int i = 0; i < pinsAvailable; ++i)
        {
          ...
          if (pinDir == PinDirection.Input)
            continue;
          //log the pin info...
          Log.Debug("analog:  capture pin:{0} {1}", i, FilterGraphTools.LogPinInfo(pins[i]));
          ...
          if (matchPinNames == false || (String.Compare(pinName, pinName1, true) == 0))
          {
            ...
            {
              //succeeded!
              Log.Debug("analog:  connected pin:{0} {1} with pin0", i, pinName);
              pinsConnected++;
            }
          ...
          if (matchPinNames == false || (String.Compare(pinName, pinName2, true) == 0))
          {
            ...
            {
              //succeeded!
              Log.Debug("analog:  connected pin:{0} {1} with pin1", i, pinName);
              pinsConnected++;
            }
            //check if all pins are connected
            if (pinsConnected == 2)
