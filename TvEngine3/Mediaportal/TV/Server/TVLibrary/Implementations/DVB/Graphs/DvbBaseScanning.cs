  ...
  public class DvbBaseScanning : IChannelScanCallBack, ITVScanning
  {
    ...
    public List<IChannel> Scan(IChannel channel, ScanParameters settings)
    {
      try
      {
        ...
        this.LogDebug("tuner locked:{0} signal:{1} quality:{2}", _card.IsTunerLocked, _card.SignalLevel,
                          _card.SignalQuality);

        _analyzer = _card.StreamAnalyzer;
        if (_analyzer == null)
        {
          this.LogDebug("no analyzer interface available");
        ...
        try
        {
          ...
          this.LogInfo("Found {0} service(s)...", serviceCount);
          ...
          for (int i = 0; i < serviceCount; i++)
          {
            ...
            {
              this.LogInfo("Service is not a TV or radio service.");
              continue;
            }
            found++;
            ...
            this.LogInfo("Found: {0}", newChannel);
            channelsFound.Add(newChannel);
          }

          this.LogInfo("Scan found {0} channels from {1} services", found, serviceCount);
    ...
    public List<IChannel> ScanNIT(IChannel channel, ScanParameters settings)
    {
      try
      {
        ...
        try
        {
          ...
          this.LogInfo("ScanNIT: tuner locked:{0} signal:{1} quality:{2}", _card.IsTunerLocked, _card.SignalLevel,
                            _card.SignalQuality);
          ...
          this.LogInfo("Found {0} multiplex(es), service information available = {1}...", multiplexCount, isServiceInfoAvailable);
          ...
          for (int i = 0; i < multiplexCount; ++i)
          {
            ...
            if (isServiceInfoAvailable)
            {
              ...
              {
                this.LogInfo("Tuning details for ONID 0x{0:x} and TSID 0x{1:x} are ambiguous, disregarding service information", originalNetworkId, transportStreamId);
          ...
          for (int i = 0; i < serviceCount; i++)
          {
            ...
            if (!multiplexesFound.ContainsKey(key))
            {
              this.LogInfo("Discarding service, no multiplex details available.");
              continue;
            }
            found++;
