...
namespace Mediaportal.TV.Server.TVLibrary.Implementations
{
  ...
  {    
    ...
    {
      this.LogDebug("load custom device plugins");
        ...
        foreach (Type type in pluginTypes)
        {
          if (type.IsClass && !type.IsAbstract)
          {
            ...
            if (cdInterface != null)
            {
              ...
              {
                this.LogDebug("skipping incompatible plugin \"{0}\" ({1})", type.Name, dllName);
              }
            }
          }
        }
      ...
      this.LogDebug("checking for supported plugins");
      _customDeviceInterfaces = new List<ICustomDevice>();
      foreach (ICustomDevice d in plugins)
      {
        ...
        if (lastFilter != null)
        {
          ...
          if (addOn != null)
          {
            this.LogDebug("add-on plugin found");
            if (!addOn.AddToGraph(ref lastFilter))
            {
              this.LogDebug("failed to add device filters to graph");
              addOn.Dispose();
              continue;
            }
            isAddOn = true;
        ...
        try
        {
          ...
          if (!isAddOn)
          {
            this.LogDebug("primary plugin found");
            break;
          }
        }
        finally
      ...
      {
        this.LogDebug("no plugins supported");
      }
    }
    ...
    {
      this.LogDebug("open custom device plugins");
      if (_useConditionalAccessInterace)
      {
    ...
    protected void ConfigurePidFilter()
    {
      ...
      foreach (ICustomDevice d in _customDeviceInterfaces)
      {
        ...
        if (filter != null)
        {
          this.LogDebug("found PID filter controller interface");
          ...
          if (pidSet == null)
          {
            this.LogDebug("assembling PID list");
    ...
    protected void UpdateDecryptList(int subChannelId, CaPmtListManagementAction updateAction)
    {
      ...
      while (en.MoveNext())
      {
        ...
        if (en.Current.SubChannelId != subChannelId && updateAction != CaPmtListManagementAction.Update)
        {
          if (updatedDigitalService != null)
          {
            ...
            {
              this.LogDebug("the service for this subchannel is a duplicate, no action required");
              return;
            }
          }
      ...
      {
        this.LogDebug("decrypt limit exceeded");
        return;
      }
      if (distinctServices.Count == 0)
      {
        this.LogDebug("no services to update");
        return;
      }
      ...
      for (int attempt = 1; attempt <= _decryptFailureRetryCount + 1; attempt++)
      {
        ...
        if (attempt > 1)
        {
          this.LogDebug("attempt {0}...", attempt);
        }
        ...
        foreach (ICustomDevice deviceInterface in _customDeviceInterfaces)
        {
          ...
          this.LogDebug("CA provider {0}...", caProvider.Name);
          foundCaProvider = true;

          if (_waitUntilCaInterfaceReady && !caProvider.IsInterfaceReady())
          {
            this.LogDebug("provider is not ready, waiting for up to 15 seconds", caProvider.Name);
            ...
            while (waitTime.TotalMilliseconds < 15000)
            {
              ...
              {
                this.LogDebug("provider ready after {0} ms", waitTime.TotalMilliseconds);
                break;
              }
            }
          }
          ...
          this.LogDebug("sending command(s)");
          bool success = false;
        ...
        if (!foundCaProvider)
        {
          this.LogDebug("no CA providers identified");
          return;
        }
      }
    }
    ...
    {
      this.LogDebug("lock in on signal");
      _tunerLocked = false;
    ...
    {
      this.LogDebug("stop, idle mode = {0}", _idleMode);
      try
      {
        ...
        foreach (ICustomDevice deviceInterface in _customDeviceInterfaces)
        {
          ...
          {
            this.LogDebug("plugin \"{0}\" overrides action {1} with {2}", deviceInterface.Name, action, pluginAction);
            action = pluginAction;
          }
          else if (action != pluginAction)
          {
            this.LogDebug("plugin \"{0}\" wants to perform action {1}, overriden", deviceInterface.Name, pluginAction);
          }
        }
    ...
    {
      this.LogDebug("perform device action, action = {0}", action);
      try
      {
        ...
        else
        {
          if (_graphBuilder == null)
          {
            this.LogDebug("graphbuilder is null");
            return;
          }
          ...
          {
            this.LogDebug("unhandled action");
            return;
          }
        }
    ...
    {
      this.LogDebug("set graph state, state = {0}", state);
      ...
      {
        this.LogDebug("graph already in required state");
        return;
      }
      int hr = 0;
      ...
      if (hr < 0 || hr > 1)
      {
        this.LogError("failed to perform action, hr = 0x{0:x} ({1})", hr, HResult.GetDXErrorString(hr));
    ...
    {
      this.LogDebug("tune channel, {0}", channel);
      try
      {
        ...
        {
          this.LogDebug("creating new subchannel");
          subChannelId = CreateNewSubChannel(channel);
        }
        else
        {
          this.LogDebug("using existing subchannel");
        ...
        this.LogInfo("subchannel ID = {0}, subchannel count = {1}", subChannelId, _mapSubChannels.Count);
    ...
    {
      this.LogDebug("subchannel {0} cancel tune", subChannelId);
      _cancelTune = true;
      if (_mapSubChannels.ContainsKey(subChannelId))
      {
    ...
    {
      this.LogDebug("free subchannel, ID = {0}, subchannel count = {1}", id, _mapSubChannels.Count);
      if (_mapSubChannels.ContainsKey(id))
      {
        if (_mapSubChannels[id].IsTimeShifting)
        {
          this.LogDebug("subchannel is still timeshifting!");
          return;
        }
        if (_mapSubChannels[id].IsRecording)
        {
          this.LogDebug("subchannel is still recording!");
          return;
        }
      ...
      {
        _subChannelId = 0;
        this.LogDebug("no subchannels present, stopping device");
        Stop();
      }
      else
      {
        this.LogDebug("subchannels still present, leave device running");
      }
    }
    ...
    {
      this.LogInfo("FreeAllSubChannels");
      Dictionary<int, BaseSubChannel>.Enumerator en = _mapSubChannels.GetEnumerator();
      while (en.MoveNext())
      {
