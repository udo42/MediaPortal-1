...
namespace Mediaportal.TV.Server.TVLibrary.Implementations
{
  ...
  public class TvCardCollection
  {
    ...
    public TvCardCollection(IEpgEvents epgEvents)
    {
      ...
      if (delayDetect >= 1)
      {
        this.LogInfo("Detecting Cards in {0} seconds", delayDetect);
        System.Threading.Thread.Sleep(delayDetect * 1000);
      }
      this.LogInfo("Detecting Cards");
    ...
    private void DetectCards()
    {
      ...
      for (int i = 0; i < devices.Length; ++i)
      {
        ...
        {
          this.LogInfo("Detected B2C2 source filter");
        ...
        else if (String.Compare(devices[i].Name, "Elecard NWSource-Plus", true) == 0)
        {
          ...
          {
            this.LogInfo("Detected Elecard IP TV Card " + cardNum);
        ...
        else if (String.Compare(devices[i].Name, "MediaPortal IPTV Source Filter", true) == 0)
        {
          ...
          for (int cardNum = 0; cardNum < iptvCardCount; cardNum++)
          {
            this.LogInfo("Detected MediaPortal IP TV Card " + cardNum);
      ...
      if (devices.Length > 0)
      {
        ...
        else
        {
          ...
          {
            this.LogError("Card detection error: {0}", ex.ToString());
          }
          ...
          for (int i = 0; i < devices.Length; i++)
          {
            ...
            this.LogInfo("Found card:{0}", name);
            ...
            if (name.Contains("silicondust hdhomerun tuner"))
            {
              ...
              this.LogInfo("silicondust hdhomerun detected - prefer cable mode: {0}", isCablePreferred);
            }
            IBaseFilter tmp;
            try
            {
            ...
            {
              //ignore bad card
              this.LogInfo("cannot add filter {0} to graph", devices[i].Name);
              continue;
            }
            ...
            if (genericNP)
            {
              ...
              {
                this.LogInfo("Detected DVB card:{0}", name);
                ...
                for (int n = 0; n < pulcNetworkTypes; n++)
                {
                  ...
                  {
                    this.LogInfo("Detected DVB-T* card:{0}", name);
                  ...
                  {
                    this.LogInfo("Detected DVB-S* card:{0}", name);
                  ...
                  {
                    this.LogInfo("Detected DVB-C* card:{0}", name);
                  ...
                  {
                    this.LogInfo("Detected ATSC* card:{0}", name);
                  ...
                  {
                    this.LogInfo("Connected with generic MS Network Provider however network types don't match, using the original method");
                  }
                }
              }
              else
              {
                this.LogInfo("Not connected with generic MS Network Provider, using the original method");
                connected = false;
              }
      ...
      for (int i = 0; i < devices.Length; i++)
      {
        string name = devices[i].Name ?? "unknown";
        name = name.ToLowerInvariant();
        this.LogInfo("Detected analog card:{0}", name);
                                                     ...
                                                     Guid networkProviderClsId, DsROTEntry rotEntry)
    {
      ...
      for (int i = 0; i < devices.Length; i++)
      {
        ...
        if (ConnectFilter(graphBuilder, networkDVB, tmp))
        {
          this.LogInfo("Detected DVB card:{0}- Hash: {1}", name, hash);
          interfaceNetworkProvider.GetAvailableTuningTypes(out tuningTypes);
          this.LogInfo("TuningTypes: " + tuningTypes);
          // determine the DVB card supported GUIDs here!
