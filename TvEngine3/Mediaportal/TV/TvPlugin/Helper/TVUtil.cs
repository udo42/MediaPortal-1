...
namespace Mediaportal.TV.TvPlugin.Helper
{
  ...
  public class TVUtil
  {
    ...
    public static string GetDisplayTitle(Schedule schedule)
    {
      string displayname = "";
      Program program =
        ServiceAgents.Instance.ProgramServiceAgent.GetProgramsByTitleTimesAndChannel(schedule.ProgramName,
                                                                                     schedule.StartTime,
                                                                                     schedule.EndTime,
                                                                                     schedule.IdChannel);
    ...
    public static void GetEpisodeInfo(StringBuilder strBuilder, string episodeName, string seriesNum, string episodeNum,
                                      string episodePart)
    {
    ...
    /// <returns></returns>
    public static void TitleDisplay(StringBuilder strBuilder, string title, string episodeName, string seriesNum,
                                    string episodeNum,
                                    string episodePart)
    {
    ...
    public static bool PlayRecording(Recording rec, double startOffset, g_Player.MediaType mediaType)
    {
      ...
      bool useRTSP = TVHome.UseRTSP();
      string chapters = useRTSP
                          ? ServiceAgents.Instance.ControllerServiceAgent.GetRecordingChapters(rec.IdRecording)
                          : null;
    ...
    public static bool DeleteRecAndSchedWithPrompt(Schedule schedule)
    {
      ...
      if (IsValidSchedule(schedule))
      {
        Program prg = ServiceAgents.Instance.ProgramServiceAgent.GetProgramByTitleAndTimes(schedule.ProgramName,
                                                                                           schedule.StartTime,
                                                                                           schedule.EndTime);
    ...
    private static bool StopRecAndDeleteSchedule(Schedule schedule, Schedule parentSchedule, int idChannel,
                                                 DateTime canceledStartTime)
    {
    ...
    private static bool CancelEpisode(DateTime cancelStartTime, Schedule scheduleToCancel, int idChannel)
    {
      ...
      if (scheduleToCancel != null)
      {
        ...
        if (!isOnce)
        {
          ...
          CanceledSchedule canceledSchedule = CanceledScheduleFactory.CreateCanceledSchedule(IdForScheduleToCancel,
                                                                                             idChannel, cancel);
    ...
    private static string GetRecordingTitle(int IdSchedule)
    {
      ...
      if (schedule != null)
      {
        Schedule spawnedSchedule = ServiceAgents.Instance.ScheduleServiceAgent.RetrieveSpawnedSchedule(IdSchedule,
                                                                                                       schedule.
                                                                                                         StartTime);
        if (spawnedSchedule != null)
        {
          recordingTitle =
            GetDisplayTitle(ServiceAgents.Instance.RecordingServiceAgent.GetActiveRecording(spawnedSchedule.IdSchedule));
        }
        else
        {
    ...
    private static void GetParentAndSpawnSchedule(ref Schedule schedule, out Schedule parentSchedule)
    {
      ...
      if (parentSchedule == null)
      {
        ...
        Schedule spawn = ServiceAgents.Instance.ScheduleServiceAgent.RetrieveSpawnedSchedule(parentSchedule.IdSchedule,
                                                                                             parentSchedule.StartTime);
        if (spawn != null)
        {
          schedule = spawn;
        }
